...
const ProductAdminPage:FC<Props> = ({ product }) => {
    ...
    const [newTagValue, setNewTag] = useState('')
    ...
    const onFileSelected = async ({target}: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            for( const file of target.files) {
                ...
                const {data} = await tiendaApi.post<{message: string}>('/admin/upload', formData)
    ...
    const onSubmit = async( form: FormData ) => {
        if(form.images.length < 2) {
            alert('Debe agregar al menos dos imagenes')
            return;
        }
        setIsSaving(true)

        try {
            const {data} = await tiendaApi({
                url: '/admin/products',
                method: form._id ? 'PUT' : 'POST', // evaluar si tenemos un _ID sino, crear un nuevo producto
                data: form
            })
            console.log(data)
    ...
    return (
        <AdminLayout 
            title={'Producto'} 
            pageDescription={`Editando: ${ product.title }`}
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={ 6 }>
                        ...
                        <TextField
                            ...
                            { ...register('price', {
                                required: 'Este campo es requerido',
                                minLength: { value: 0, message: 'Valor minimo es cero' }
                            })}
                            error={ !!errors.price }
                    ...
                    <Grid item xs={12} sm={ 6 }>
                        <TextField
                            ...
                            { ...register('slug', {
                                required: 'Este campo es requerido',
                                validate: (val) => val.trim().includes(' ') ? 'No puede contener espacios' : true
                            })}
                            error={ !!errors.slug }
                        ...
                        <Box display='flex' flexDirection="column">
                            ...
                            <Chip 
                                ...
                                color='error'
                                variant='outlined'
                                sx={{ displat: getValues('images').length > 1 ? 'none' : 'flex' }}
                            />
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
    ...
    if(slug === 'new') {
        const tempProduct = JSON.parse(JSON.stringify(new Producto()));
        delete tempProduct._id;
        tempProduct.images = ['img1.jpg', 'img2.jpg'];
        product = tempProduct;
    }else{
        product = await getProductsBySlug(slug.toString());
    }
